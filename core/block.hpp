...
#define PESSIMISTIC_DELTA_BLOCK true
    ...
    public:
        ...
        void deletion_immediate_validation(delta_chain_id_t delta_chain_id,uint32_t new_delta_offset){
            auto &target_chain_index_entry = delta_chains_index->at(delta_chain_id);
            target_chain_index_entry.update_offset(new_delta_offset| LOCK_MASK);
        }
        Delta_Chain_Lock_Response simple_set_protection_on_delta_chain(delta_chain_id_t delta_chain_id,
                                                                       std::unordered_map<uint64_t, int32_t> *lazy_update_map_ptr,
        ...
        BaseEdgeDelta* log_redo_find_previous_delta(vertex_t vid, uint32_t start_offset, uint32_t* previous_delta_offset) {
            while(start_offset>0){
                auto edge_delta = get_edge_delta(start_offset);
                if(edge_delta->toID == vid){
                    *previous_delta_offset = start_offset;
                    return edge_delta;
                }else{
                    start_offset = edge_delta->previous_offset;
                }
            }
            return nullptr;
        }
